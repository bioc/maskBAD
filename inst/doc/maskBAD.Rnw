...
external mask (here we use the object sequenceMask, containing the
information whether the probe sequence is affected by a SNP between
the groups (0) or not (1) ):
