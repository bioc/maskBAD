...
\Rfunction{ReadAffy()} from package \Rpackage{affy}. We'll use 50
...
## load(system.file("data/AffyBatch.rda", package="mask"))
## load(system.file("data/exmask.rda", package="mask"))
@
...
The scoring of probes will be performed with the function
\Rfunction{mask} and is based on the algorithm we presented in
Dannemann et. al (\cite{Dannemann09}). Briefly, for a given probe, its
signal is proportional to the amount of RNA in the sample and its
binding affinity. When one target is measured with several probes (a
probe set), as on Affymetrix arrays, the probes' signal is correlated
for each sample. BAD probes correlation differs between groups, hence
...
out lower 22\% of quality scores, we have to put a cutoff at 0.022.
...
might help to decide about a cutoff. It plots signal intensity of a
...
probesToSee = rownames(exmask[[1]][exmask[[1]]$quality.score<0.023 
  & exmask[[1]]$quality.score >0.021,])
...
\texttt{x} and \texttt{y} coordinates for the probes, along with their
BAD status coded as 1/0 (BAD probe/not BAD probe).  For a given
cutoff, compare BAD status according to expression-based mask and
external mask:
...
Here, 709 probes are concordant between masks, whereas 51 probes with
...
\begin{figure}
  \centering
  \includegraphics[width=.7\textwidth]{mask-figErr}
...
\begin{figure}
  \centering
  \includegraphics[width=\textwidth]{mask-figWil1}
  \includegraphics[width=\textwidth]{mask-figWil2}
...
\begin{figure}
  \centering
  \includegraphics[width=.7\textwidth]{mask-figHist}
